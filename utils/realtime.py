...
class RealtimeWavDataset(speechset.WavDataset):
    ...
    def trim(self,
             ...
             db: float = 60.,
             frame: int = 2048,
             hop: int = 512) -> torch.Tensor:
        """Trimming the silence.
        """
        # [T + frame]
        audio = F.pad(audio, [frame // 2, frame // 2])
        # [T / hop, frame]
        frame = audio.unfold(-1, frame, hop)
        # [T / hop]
        rms = (frame ** 2).mean(dim=-1).sqrt()
        # db to amp
        thold = 10 ** (-db / 20)
        # [N]
        nonzero = (rms > thold).nonzero()
        if len(nonzero) == 0:
            return audio
        # positioning
        start = nonzero[0].item() * hop
        end = min(audio.shape[-1], (nonzero[-1].item() + 1) * hop)
